...
spanElement.innerHTML = `${content} Sergio`;

//* Refactorizado
//*-----------------------------------------------------------------|
//document.querySelector('#createdBy').innerHTML += ' Sergio';      |
//* ----------------------------------------------------------------|
...
function buildToDo(todo, index) {
	...
	toDoShell.appendChild(btnRemove);
...
function addToDo() {
	...
	//*! preguntar como utilizar toDoInput
	let inp = document.querySelector('#toDoInput');
	if (inp.value) {
		let toDoNew = new ToDo(inp.value);
...
addButton.addEventListener('click', (e) => {
	e.preventDefault();
	addButton.classList.add('listo');
	addButton.innerHTML = '';

	setTimeout(() => {
		addButton.classList.remove('listo');
		addButton.innerHTML = '<span>AGREGAR</span>';
	}, 1000);
});
